...
export default function ProductTemplate({product}:{product:Product}){
  ...
  return <main className="product-page"><script type="application/ld+json" dangerouslySetInnerHTML={{__html:JSON.stringify(schema)}}/>
    <section className="product-hero"><div className="global-shell product-hero-grid"><div className="product-hero-copy"><span>{product.eyebrow.toUpperCase()}</span><h1>{product.name}</h1><p>{product.description}</p><WhatsAppLink href={whatsappUrl(message)} target="_blank" rel="noreferrer">Solicitar cotação</WhatsAppLink></div><div className="product-visual"><Image src={`/brand/hero-${product.slug}.png`} alt={`${product.name} com atendimento da Vital Corretora`} fill priority sizes="(max-width: 950px) 100vw, 48vw"/></div></div></section>
    ...
    <section className="product-blog-link"><div className="global-shell"><div><span>CONTEÚDO ESPECIALIZADO</span><h2>100 guias sobre {product.name}</h2><p>Contratação, comparação, custos e atendimento em Minas Gerais.</p></div><a href={`/blog/${product.slug}/`}>Abrir blog deste produto →</a></div></section>
